    ...
    pub fn purchase_nft(ctx: Context<PurchaseNft>, payment_method: PaymentMethod) -> Result<()> {
        ...
        match payment_method {
            ...
            PaymentMethod::SPLToken { token_mint, amount } => {
                // Transfer SPL Token
                let token_program = &ctx.accounts.token_program;
                let token_account_from = &ctx.accounts.buyer_token_account.ok_or(ErrorCode::OwnershipError)?;
                let token_account_to = &ctx.accounts.seller_token_account.ok_or(ErrorCode::OwnershipError)?;
